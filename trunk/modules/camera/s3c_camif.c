...
{
	//printk("s3c_camif_get_fimc_object called \n"); DEBUG
...
{
	//printk("s3c_camif_get_frame_num \n"); DEBUG
...
{
	//printk("s3c_camif_get_fifo_status \n"); debug
...
{
	printk("s3c_camif_request_memory \n");
...
int s3c_camif_set_source_format(camif_cis_t *cis)
{
	...
	if (cis->itu_fmt & CAMIF_ITU601){
		cmd = CAMIF_ITU601;
		printk("s3c_camif_set_source_format = ITU601 \n");}
	else {
		assert(cis->itu_fmt & CAMIF_ITU656);
		cmd = CAMIF_ITU656;
		printk("s3c_camif_set_source_format = ITU656 \n");
	}
 ...
 * */
	...
	if(cfg->sc.scalerbypass)
	{
		printk("!#$ CMD	cis->order422 >> 14 \n");
		cmd = (cis->order422 >> 14);
		writel((readl(cfg->regs + S3C_CICOCTRL) & ~(0x3 << 0)) | cmd, cfg->regs + S3C_CICOCTRL);
		printk("##1 ORDERING FOR JPEG cmd = %x \n",cmd); //MARC HERE CE131 CHANGE
	}
	else
	{
...
int s3c_camif_control_global_capture(camif_cfg_t *cfg,int value)
{
	...
	if(value) // enable
		val |= S3C_CIIMGCPT_IMGCPTEN;
	else  // disable
		val &= ~(S3C_CIIMGCPT_IMGCPTEN);
	writel(val, cfg->regs + S3C_CIIMGCPT);
}
//#endif
...
/*#ifndef CONFIG_VIDEO_SAMSUNG_CE131 //NOT DEFINED --> LAST m: by enabled, was disabled
		while(1)
		{
...
static int s3c_camif_set_gpio(void)
{
	...
	s3c_gpio_setpull(S3C64XX_GPF(6), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(7), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(7), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(8), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(8), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(9), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(9), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(10), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(10), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(11), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(11), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(12), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(12), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(2), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(2), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(4), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(4), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(1), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S3C64XX_GPF(1), S3C_GPIO_PULL_NONE);
	s3c_gpio_cfgpin(S3C64XX_GPF(0), S3C_GPIO_SFN(2));
...
//#ifdef CONFIG_VIDEO_SAMSUNG_CE131
EXPORT_SYMBOL(s3c_camif_control_global_capture);
//#endif
EXPORT_SYMBOL(s3c_camif_get_fimc_object);
EXPORT_SYMBOL(s3c_camif_set_polarity);
EXPORT_SYMBOL(s3c_camif_get_frame);
EXPORT_SYMBOL(s3c_camif_start_preview_msdma);
EXPORT_SYMBOL(s3c_camif_change_flip);
EXPORT_SYMBOL(s3c_camif_change_effect); 
EXPORT_SYMBOL(s3c_camif_control_fimc);
EXPORT_SYMBOL(s3c_camif_set_source_format);
EXPORT_SYMBOL(s3c_camif_stop_dma);
EXPORT_SYMBOL(s3c_camif_start_codec_msdma);
EXPORT_SYMBOL(s3c_camif_start_dma);
EXPORT_SYMBOL(s3c_camif_reset);
EXPORT_SYMBOL(s3c_camif_init);
EXPORT_SYMBOL(s3c_camif_get_frame_num);
EXPORT_SYMBOL(s3c_camif_get_fifo_status);
EXPORT_SYMBOL(s3c_camif_clear_irq);
EXPORT_SYMBOL(s3c_camif_set_priority);
