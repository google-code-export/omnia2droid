...
static unsigned short ce131_normal_i2c[] = { (ce131_ID >> 1), I2C_CLIENT_END };
static unsigned short ce131_ignore[] = { I2C_CLIENT_END };
static unsigned short ce131_probe[] = { I2C_CLIENT_END };


static struct i2c_client_address_data ce131_addr_data = {
	.normal_i2c = ce131_normal_i2c,
	.ignore		= ce131_ignore,
	.probe		= ce131_probe,
};
...
static int ce131_attach(struct i2c_adapter *adap, int addr, int kind)
{
	printk("functie! ce131_attach \n");
	(printk("[CAM-SENSOR] +%s\n",__func__));
	struct i2c_client *c;
	...
	c->addr = addr;
	c->adapter = adap;
	c->driver = &ce131_driver;
	ce131_data.sensor = c;

	return i2c_attach_client(c);
}

static int ce131_sensor_attach_adapter(struct i2c_adapter *adap)
{
	printk("functie! ce131_sensor_attach_adapter\n");
	return i2c_probe(adap, &ce131_addr_data, ce131_attach);
}

static int ce131_sensor_detach(struct i2c_client *client)
{
	printk("functie! ce131_sensor_detach\n");
	(printk("[CAM-SENSOR] +%s\n",__func__));
...
{
	int err;
	//printk("ce131_set_metering type = %x \n", type);
...
static struct i2c_driver ce131_driver = {
	...
	.id = ce131_ID,
	.attach_adapter = ce131_sensor_attach_adapter,
	.detach_client = ce131_sensor_detach,
